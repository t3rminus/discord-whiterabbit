...
class BotBase {
  constructor () {
    ...
    // Log errors
    this.bot.on('error', (err) => console.erroror(err));
    ...
    this.db.on('error', err => {
      console.erroror('An error ocurred connecting to redis.', err);
    });
  }
  ...
  async handleJoin (member) {
    ...
    if (serverSettings.welcome_message && serverSettings.welcome_channel) {
      const welcome = serverSettings.welcome_message
  ...
  async displayHelp (message, prefix, postfix, localMessage) {
    ...
    for (const commandName of commands) {
      ...
      if (!command.adminOnly || isAdmin) {
        const name = command.ignorePrefix ? `?${commandName}` : `${settings.prefix + commandName}`;
  ...
  async isAdmin (message) {
    ...
    if (settings.admin_group) {
      ...
      const roles = message.channel.guild.roles.filter((role) => {
        return settings.admin_group.indexOf(role.name) > -1;
      });

      if (roles.size > 0) {
        const hasARole = roles.some(role => {
  ...
  async getSetting (member, key) {
    ...
    try {
      return JSON.parse(await this.db.get(dataKey));
    } catch (err) {
      console.log(err);
      return null;
    }
  ...
  async sendReply (message, content, options) {
    const channel = (message && message.channel) || message;
    await Misc.delay(200);
