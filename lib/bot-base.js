...
class BotBase {
					...
					if(matchCmd.test(message.content)) {
						if(commandInfo.method && isFunction(this[commandInfo.method])) {
							...
							if(commandInfo.parseParams === false) {
								...
								params = YargsParser(message.content.replace(matchCmd, ''));
							}
	...
	findUsers(search, message) {
		return Bluebird.try(() => {
			...
			const quickFind = Misc.stringNormalize(search.join());
			const exactShortcut = message.channel.guild.members.find((member) =>
				Misc.stringNormalize(member.displayName) === quickFind
			);
			...
			const startShortcut = message.channel.guild.members.filter((member) =>
				Misc.stringNormalize(member.displayName).indexOf(quickFind) === 0
			);
			if(startShortcut.length === 1) {
				return plural ? startShortcut : startShortcut[0];
			}
	...
	fail(message) {
		return this.getServerSettings(message)
			.then(settings => {
				...
				return message.channel.send(fail_messages[which]);
			});
	}
