...
const Schedule = require('node-schedule'),
	...
	RSSMixin = require('../mixins/rss-mixin');
...
class WhiteRabbit extends Mix(BotBase)
	.with(DiceMixin, TimezoneMixin, DnDSpellMixin,
		...
		CatMixin, LocationMixin, RSSMixin) {
		...
		for (const [, guild] of this.bot.guilds) {
			if (guild.available) {
				...
				if (name !== null && member.nickname === null && eventStart) {
					await member.setNickname(finalName).catch(() => { /* ignore error */ });
				} else if (finalName === member.nickname && !eventStart) {
					await member.setNickname(null).catch(() => { /* ignore error */ });
				}
			}
		}
	}
	...
	async phrase__jabberwocky(message) {
		...
		await this.saveSetting(message.member, '-jabberwocky', (phrase + 1) % jabberwocky.length, true);
	...
	async command__whiterabbitcfgdump(params, message) {
		if (await this.isAdmin(message)) {
			...
			const filename = `${message.guild.name.toLowerCase().replace(/[^a-z]/g, '-')}_config.json`;
			const attachment = new BotBase.Discord.Attachment(Buffer.from(cfgDump, 'utf8'), filename);
			message.author.send(`Psst! Here’s the configuration for ${message.guild.name}`, attachment);
		}
	}
