...
class WhiteRabbit extends Mix(BotBase)
	...
	scheduleEvents() {
		return Bluebird.try(() => {
			...
			return this.setAvatar(curIcon, true);
		});
	}
	
	setAvatar(icon, start) {
		icon = icon || {};
		return Bluebird.try(() => {
			...
			return this.bot.user.setAvatar(image).catch((err) => { console.error(err.message); });
		})
		.then(() => {
	...
	botReady() {
		...
		Schedule.scheduleJob('1 0 0 1 1 *', () => { this.scheduleEvents.call(this) });
		readyTracker.push(this.scheduleEvents());
