	...
	whenisAll(message) {
		return this.getSetting(message.member, '-timezones')
			.then((data) => {
				...
				timeZones.sort((a,b) => {
					return moment.tz.zone(a).offset(Date.now()) - moment.tz.zone(b).offset(Date.now());
				});
				
				timeZones.forEach((timezone) => {
					const userData = data[timezone];
					let resultMessage = '**' + moment().tz(timezone).format('h:mma') + ':** '
						+ (userData.users.map(u => message.guild.members.get(u))
							.filter(u => !!u).map(u => u.displayName).join(', '));
					if(userData.count > 50) {
						resultMessage += ' …and ' + (userData.users.length - 50) + ' more';
					}
