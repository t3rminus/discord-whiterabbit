...
let spellCache, normalizedSpellCache;
...
module.exports = (BotBase) =>
  class DnDSpellMixin extends BotBase {
    ...
    async command__spell5e (params, message) {
      const search = params.trim().toLowerCase().replace(/[^a-z0-9 \-'’]+/g, '');

      const spells = await DnDSpellMixin.getSpellInfo();
      ...
      const spellInfo = spells[result.value];
      ...
      return this.sendReply(message, { embed: reply });
    }
      ...
      if (!spellCache) {
        spellCache = fs.readFile(localFile)
          ...
          .then((spells) => {
            ...
            return spells.reduce((out, spell) => {
              ...
              const components = { verbal: false, somatic: false, material: false };
              if (spell.components) {
